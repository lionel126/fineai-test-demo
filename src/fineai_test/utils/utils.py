...
def jwt_token(iss='http://192.168.103.101:9090'):
    ...
    app_id = 'wxee64f42a910fe3e6' #:csgoIRU76zO7uMhAy3wCV0Y49F5De5k:csgoA_WQ63tqac75U0iL6tIUWYQhmow'

    login_user_key = hashlib.md5(bytes(f'{app_id}:{union_id}:{open_id}', 'utf-8')).digest().hex()
    sub = "XW IT Co."
    # exp = datetime.now()
    salt = 'xinPC_MA@2023'
    h = Hashids(salt=salt, min_length=16)
    
    payload = {"unionId": h.encode(user_id), "sub": sub, "iss": iss, "login_user_key": login_user_key}
    # print(payload)
